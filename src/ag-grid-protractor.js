...
exports.getLocatorForCell = function(row, col, additionalSelector) {
	return protractor_1.by.css(
		exports.getCssSelectorForRowAndCol(row, col, additionalSelector)
	);
};
exports.getCellContent = function(row, col, additionalSelector) {
	return protractor_1.element(
		exports.getLocatorForCell(row, col, additionalSelector)
	);
};
exports.getCellContentAttribute = function(
	row,
	col,
	attribute,
	additionalSelector
	...
	return exports
		.getCellContent(row, col, additionalSelector)
		.getAttribute(attribute);
};
exports.verifyCellContentAttributesContains = function(
	row,
	col,
	attribute,
	values,
	...
	exports
		.getCellContentAttributes(row, col, attribute, additionalSelector)
		.then(function(attributes) {
			exports.log(
				"Verifying " + values.length + " equals " + attributes.length
			);
			expect(values.length).toEqual(
				attributes.length,
				"Expect number of values supplied (" +
					values.length +
					") to equal number of attributes found (" +
					attributes.length +
					"), for row " +
					row +
					", col " +
...
exports.getCellContentAttributes = function(
	row,
	col,
	attribute,
	additionalSelector
	...
	return protractor_1.element
		.all(exports.getLocatorForCell(row, col, additionalSelector))
		.map(function(elm, index) {
			return elm.getAttribute(attribute);
		});
};
exports.verifyCellContentAttributeContains = function(
	row,
	col,
	attribute,
	value,
	...
	exports
		.getCellContentAttribute(row, col, attribute, additionalSelector)
		.then(function(text) {
			exports.log("Expecting " + text + " to contain " + value);
			expect(text).toContain(
				value,
				"Expecting " + text + " to contain " + value
			);
		});
};
...
exports.verifyCellContentsMatches = function(
	rowIndex,
	colId,
	expectedValue,
	additionalSelector
	...
	exports
		.getCellContentsAsText(rowIndex, colId, additionalSelector)
		.then(function(text) {
...
exports.verifyRowDataMatchesGridData = function(exectedRowData) {
	var _loop_1 = function(rowIndex) {
		...
		colIds.forEach(function(colId) {
			exports.verifyCellContentsMatches(
				rowIndex,
				colId,
				currentRowData[colId]
			);
		});
	};
...
exports.verifyElementIsPresent = function(locator) {
	...
	return expect(protractor_1.element(locator).isPresent()).toBeTruthy(
...
exports.verifyElementIsNotPresent = function(locator) {
	...
	expect(protractor_1.element(locator).isPresent()).toBeFalsy(
		"Expect element " + locator + " to not be present"
	);
};
...
exports.allElementsByMatch = function(locator, action, comparisonFn) {
	return exports.mapAllElementsBy(locator, action).then(function(item) {
		return comparisonFn(item);
	});
};
exports.allElementsTextMatch = function(locator, values) {
	exports.allElementsByMatch(
		locator,
		function(item) {
			return item.getText();
		},
...
exports.clickOnHeader = function(headerName) {
	return protractor_1
		.element(
			protractor_1.by.cssContainingText(
				".ag-header-cell-text",
				headerName
			)
		)
